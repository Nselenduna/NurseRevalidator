...
export default function RegistrationScreen() {
  ...
  useEffect(() => {
    ...
    if (passwordInput && passwordInput.focus && scrollViewRef.current) {
      // Store the original focus method
      const originalFocus = passwordInput.focus.bind(passwordInput);
    ...
    return () => {
      // Clean up if needed
      if (passwordRef.current && TextInput.prototype.focus) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      navigation.navigate(ROUTES.ProfessionalDetails as never);
    } catch (error) {
